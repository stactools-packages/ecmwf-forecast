...
STEPS = ([f"{i}h"
          for i in range(0, 144, 3)] + [f"{i}h" for i in range(144, 361, 6)] +
         [f"{i}m"
          for i in range(1, 8)]  # TODO: double check this endpoint, and below
         )
STEP_UNITS = ["h", "m"]
TYPES = ["fc", "ef", "ep", "tf"]
FORMATS = ["grib2", "bufr"]
...
def get_combinations(fmt="grib2"):
    ...
    if fmt != "grib2":
        raise NotImplementedError(
            "Only 'grib2' format is supported currently.")
            ...
            for type_ in TYPES:
                ...
                if reference_time in {"00", "12"}:
                    if stream in {"enfo", "waef"}:
                        if type_ == "ef":
                            combinations.extend([
                                Combination(fmt, type_, reference_time, stream,
                    ...
                    if stream in {"enfo", "waef"}:
                        if type_ == "ef":
                            combinations.extend(
                                ...
                                [
                                    Combination(fmt, type_, reference_time,
                                                stream, f"{i}h")
